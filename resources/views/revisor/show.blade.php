<x-layout>
    <x-header title="{{__('ui.annuncio')}}: {{$product->name}}"/>
        <div class="container my-2 my-md-3 pb-5 container-annunci-custom rounded-5">
            <div class="row justify-content-center mb-4">

                <div class="col-12 col-md-5 mt-1 mt-md-4  border-end">
                    <h5 class="text-uppercase fw-bold fs-5 bg-body-tertiary">{{__('ui.attribTags')}}:</h5>
                    <ul class="gap-3 d-flex flex-wrap ps-0 mb-0 justify-content-center bg-body-tertiary py-2">
                        @foreach ($product->images as $image)
                        @foreach ($image->labels as $label)
                        <li class="list-group-item d-inline border rounded-pill px-2">
                            {{ $label }}
                        </li>
                        @endforeach
                        @endforeach
                ...
                <div class="col-12 col-md-4 mt-3 mt-md-4 px-2 pb-2">
                    <h5 class="text-uppercase fw-bold fs-5  bg-body-tertiary">{{__('ui.revisImg')}}:</h5>
                    <ul class="d-flex flex-wrap justify-content-center bg-body-tertiary py-2 ps-0 text-center gap-4">
                        @foreach ($product->images as $image)
                        <li class="list-group-item">Adult: <i class="{{ $image->adult }}"></i></li>
                        <li  class="list-group-item">Medical: <i class="{{ $image->medical }}"></i></li>
                        <li  class="list-group-item">Spoof: <i class="{{ $image->spoof }}"></i></li>
                        <li  class="list-group-item">Violence: <i class="{{ $image->violence }}"></i></li>
                        <li  class="list-group-item">Racy: <i class="{{ $image->racy }}"></i></li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <div class="row justify-content-center flex-row align-items-between">
                ...
                <div class="col-12 col-md-4 mt-md-4 ms-2 p-2 d-flex gap-2 justify-content-end justify-content-md-center align-items-center bg-body-tertiary">
                    ...
                    @elseif ($product->is_accepted != NULL && $product->is_accepted == false)
                    <form action="{{route('revisor.accept', compact('product'))}}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn rounded-pill btn-custom-color1">{{__('ui.approva')}}</button>
                    </form>
                    @elseif ($product->is_accepted != NULL && $product->is_accepted == true)
                    <form action="{{route('revisor.delete', compact('product'))}}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn rounded-pill btn-light border-danger bg-light border-2 text-danger btn-declina">{{__('ui.declina')}}</button>
