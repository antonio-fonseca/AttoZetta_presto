<x-layout>
    <x-header title="{{__('ui.annuncio')}}: {{$product->name}}"/>
      <div class="container mt-2 mt-md-5 pb-5 container-annunci-custom rounded-5 shadow">
        <div class="row justify-content-center flex-row align-items-between">
            ...
            <div class="col-12 col-md-4 mt-4 mt-md-0 d-flex flex-column justify-content-between">
                    ...
                    <div class="d-flex justify-content-between flex-column">
                      <p class="text-uppercase ps-1">{{__('ui.categoria')}}: <strong>{{__("ui." . $product->category->name)}}</strong> </p>
            ...
            <div class="col-12 col-md-4 d-flex mt-4 bg-body-tertiary p-3 me-md-4 flex-column justify-content-end align-items-end gap-1">

              @if (Auth::user() == $product->user)

              <a href="{{route('products.edit', $product)}}" class="ps-3 text-color3"><span class="fw-bold">{{__('ui.modifica')}}</span><i class="ps-2 pb-0 text-color2 fa-solid fa-pen-to-square icon-anim fs-5"></i></a>
              <livewire:product-delete product_id="{{$product->id}}"/>
